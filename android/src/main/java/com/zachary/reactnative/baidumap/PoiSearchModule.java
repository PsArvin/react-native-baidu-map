...
public class PoiSearchModule extends BaseModule implements OnGetPoiSearchResultListener, OnGetSuggestionResultListener {
  ...
  public void searchInCityProcess(String city, String keyword, int pageNum) {
    ...
    searchType = 1;
    mPoiSearch.searchInCity((new PoiCitySearchOption()).city(city).keyword(keyword).pageNum(pageNum));
  }
  ...
  public void searchNearbyProcess(String keyword, double centerLat, double centerLng, int radius, int pageNum) {
    ...
    PoiNearbySearchOption nearbySearchOption = new PoiNearbySearchOption().keyword(keyword)
        .sortType(PoiSortType.distance_from_near_to_far).location(new LatLng(centerLat, centerLng)).radius(radius)
        .pageNum(pageNum);
   ...
   */
  @ReactMethod
  public void searchBoundProcess(String keyword, double southwestLat, double southwestLng, double northeastLat,
      double northeastLng) {
    searchType = 3;
    ...
    mPoiSearch.searchInBound(new PoiBoundSearchOption().bound(searchBound).keyword(keyword));
  }
